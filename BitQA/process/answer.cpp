...
BitQA::Answer::Answer(int AnswerID)
{
	...
	while (res->next()) {
		...
		this->AnswerOwner = res->getInt("aowner");
	}
...
vector<BitQA::Comment> BitQA::Answer::getComments()
{
	...
	res = stmt->executeQuery("SELECT *, tblComment.id FROM tblComment WHERE tblComment.parentContentId IN (SELECT tblAnswer.contentId FROM tblAnswer WHERE id = '" + to_string(this->AnswerID) + "')");
...
string BitQA::Answer::getUsername()
{
	...
	res = stmt->executeQuery("SELECT displayname FROM tblUser WHERE username = '" + to_string(this->AnswerOwner) + "'");
